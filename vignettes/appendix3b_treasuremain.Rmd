...
ext_bin_data <- function(
  ...
  subj_e     =   .05, # by-subject random slope sd for exp
  subj_c     =   .05, # by-subject random slope sd for condition
...
return(list(mydat,n_subj, n_condR, n_condN, beta_0, beta_e, beta_c, beta_ec, item_0, item_e, item_rho, subj_0, subj_e, subj_c, subj_ec, subj_rho))
}
```
...
cat_exp <- est[[4,2]]
data.frame(
  T1_condN = inv_logit(int + .25*cat + .25*exp + .25*cat_exp),
  T1_condR  = inv_logit(int - .25*cat + .25*exp - .25*cat_exp),
  T2_condN = inv_logit(int + .25*cat - .25*exp - .25*cat_exp),
  T2_condR  = inv_logit(int - .25*cat - .25*exp + .25*cat_exp)
) %>%
  gather(key, val, 1:4) %>%
...
```{r power}
powersummary <- data.frame(matrix(NA,nrow=length(alln),ncol=23))
colnames(powersummary) <- c('reps','n_subj', 'n_condR', 'n_condN', 'beta_0', 'beta_e', 'beta_c', 'beta_ec', 'item_0', 'item_e', 'item_rho', 'subj_0', 'subj_e', 'subj_c', 'subj_ec', 'subj_rho','A1B1','A1B2','A2B1','A2B2','powerC','powerE','powerCE')
...
for (j in 1:15){
  powersummary[i,j]<-datlist[[j]]
}
powersummary$reps[i]<-reps
powersummary$n_subj[i] <- n_subj
powersummary$subj_rho[i]<-datlist[[16]][1] #just 1st value; we won't be setting this!
powersummary$A1B1[i] <- A1B1
powersummary$A1B2[i] <- A1B2
powersummary$A2B1[i] <- A2B1
powersummary$A2B2[i] <- A2B2

powersummary$powerC[i] <- round(psimbit$x[2],4)
powersummary$powerE[i] <- round(psimbit$x[3],4)
powersummary$powerCE[i] <- round(psimbit$x[4],4)
